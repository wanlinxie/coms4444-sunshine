...
public class Player implements sunshine.sim.Player 
{
	...
	private ArrayList<List<Point>> groupings = new ArrayList<List<Point>>(); 
	private int groupNum = 0; 
	private HashMap<Integer, List<Integer>> tractorToTrailers = new HashMap<Integer, List<Integer>>(); 
	private HashMap<Integer, List<Integer>> groupToTrailer = new HashMap<Integer, List<Integer>>(); 
	...
	public void init(List<Point> bales, int n, double m, double t)
	{
		...
		System.out.println("Groupings size" + groupings.size()); 
        ...
        private void assignGroups(){
        	...
        	List<Point> temp = groupings.get(0); 
        	for(Point p : temp){
  ...
  //                    			trailer_bales.remove(trailer_bales.indexOf(tasks.get(i)));
		...
		if (tractor.getLocation().equals(BARN)) 
		{
			...
			else if (tractor.getAttachedTrailer() != null) //trailer
			{
				...
				{
					System.out.print("Size of task list for tractor: " + taskList.get(tractor.getId()).size());  
					//either move if 
					if ((taskList.get(tractor.getId()).size()) > 0) 
					{
						//tractor has tasks
						...
						System.out.print("Printing point 1:" + p); 
						//TODO
		...
		else
		{
			...
			else 
			{
				...
				if(groupings.get(0).size() > 11){
					...
					temp = tractorToTrailers.get(tractor.getId()).get(2);
					trail_loc = trailer_map.get(temp); 
				}
